...
public class HtmlRenderController {
    ...
    @GetMapping("{fileId}")
    public ResponseEntity<UploadFileResponse> renderHtml(@PathVariable("fileId") String fileId) {
        try {
            String responseId = profileReportService.generateHTMLFile(fileId);
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(new UploadFileResponse("", e.toString(), "", "", 0));
        }
